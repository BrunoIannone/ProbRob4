...
% [X,chi]=oneRound([NOMINAL_PARAMS,SENSOR_TRANSLATION_WRT_ROBOT],[absolute_values,incremental_values_rel,sensor_gt_rel(1:end-1,:)], N_ITERATIONS,ENCODER_MAX_VALUES);

% calibrated_my_robot_odometry = compute_odometry_trajectory(stack_odometry(X(1:4), [absolute_values, incremental_values_rel], ENCODER_MAX_VALUES));
...
% if plot_
%     h6 = my_plot(calibrated_my_robot_odometry,calibrated_sensor_odometry_plot, 2, 'g-');
%     h7 = my_plot(calibrated_my_sensor_odometry,calibrated_sensor_odometry_plot, 2, 'm-');
